...
I analyzed **103,904 airline passenger survey responses** to identify who is most dissatisfied, what drives that dissatisfaction, and what it costs the airline in lost revenue. A Random Forest model predicts dissatisfaction with **94.48% accuracy** — beating the 56.6% baseline by 37.88 percentage points.

- **$14.7M in revenue is at risk** from dissatisfied passengers
- **Business class accounts for 51.6%** of that revenue at risk — despite having the *lowest* dissatisfaction rate, proving dissatisfaction rate alone is a misleading metric
- **Online boarding is the #1 driver of dissatisfaction**, confirmed independently by both correlation analysis and ML feature importance

> **Bottom line:** Fixing online boarding has the highest financial return of any single intervention, because — unlike class-specific fixes — it affects every passenger segment equally.
...
- **Source:** [Airline Passenger Satisfaction](https://www.kaggle.com/datasets/teejmahal20/airline-passenger-satisfaction) by TJ Klein (Kaggle)
- **Scope:** 103,904 training records · 25,976 test records
...
1. Clone the repository:
   ...
   ```
2. Download the dataset from [Kaggle](https://www.kaggle.com/datasets/teejmahal20/airline-passenger-satisfaction) and place `train.csv` and `test.csv` in the `data/` folder.
...
- Economy class passengers have an **81.4% dissatisfaction rate** — 2.7× higher than Business class (30.6%)
...
### Can we predict dissatisfaction? (BQ3)
...
- **Precision: 95%** — when the model flags a passenger as dissatisfied, it's right 95% of the time
- Both correlation analysis and ML feature importance independently rank online boarding #1 — two different methods agreeing is strong evidence

### What is the financial cost? (BQ4)
...
- Fixing online boarding would have the highest financial return, since it affects all classes equally

**Putting it together:** dissatisfaction isn't evenly spread across passengers, but its root cause is. Online boarding is the one factor affecting every class and age group equally — which is exactly why fixing it, not any single class-specific change, has the highest financial return.

## ⚠️ Limitations

- **Feature importance ≠ causation:** Online boarding ranking #1 in both methods is strong evidence of association, but doesn't prove fixing it alone would reduce dissatisfaction by a specific amount — a controlled test would confirm the causal impact.
- **Revenue-at-risk assumptions:** The $14.7M figure uses estimated industry-average revenue per passenger (Business: $500, Eco Plus: $250, Eco: $150), since actual ticket price data wasn't available in this dataset. Real-world figures would vary by route, season, and rebooking behavior.
...
├── dashboard/
│   ├── airline_dashboard.pbix    # Power BI dashboard (3 pages)
│   └── screenshots/
